...
import properties.Unit;

public class XMLParser {

  public Map<String, String> getMapOfProperties(JAXBElement<properties.Qproperties> root) {
  ...
  public List<Property> getAllProperties(JAXBElement<properties.Qproperties> root) {
    List<Property> res = getProperties(root);
    res.addAll(getExpFactors(root));
    return res;
  }

  public List<Property> getProperties(JAXBElement<properties.Qproperties> root) {
  ...
  public List<Property> getExpFactors(JAXBElement<properties.Qproperties> root) {
    List<Property> res = new ArrayList<Property>();
    if (root.getValue().getQfactors() != null) {
      Qfactors factors = root.getValue().getQfactors();
      for (Qcategorical cat : factors.getQcategorical()) {
  ...
  public JAXBElement<properties.Qproperties> addProperties(JAXBElement<properties.Qproperties> root,
      List<Property> props) {
    // root of properties that are not experimental factors
  ...
  /**
   * returns experimental factors as well as other properties
   * 
   * @param xml
   * @return
  ...
  /**
   * only returns experimental factors (no other properties)
   * 
   * @param xml
   * @return
   * @throws JAXBException
  ...
  /**
   * only returns properties (as opposed to experimental factors)
   * 
   * @param xml
   * @return
   * @throws JAXBException
  ...
  public JAXBElement<properties.Qproperties> getEmptyXML() throws JAXBException {
    ...
    JAXBElement<Qproperties> root = unmarshaller.unmarshal(new StreamSource(
        new StringReader("<?properties version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>"
